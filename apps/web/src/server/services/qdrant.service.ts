...
// Collection 命名规则：assistant_{assistantId}
const getCollectionName = (assistantId: string): string => `assistant_${assistantId}`;
 ...
 */
export async function createCollection(assistantId: string): Promise<void> {
  const client = getClient();
  const collectionName = getCollectionName(assistantId);
 ...
 * @param assistantId 助手 ID
 */
export async function deleteCollection(assistantId: string): Promise<void> {
  const client = getClient();
  const collectionName = getCollectionName(assistantId);
...
export async function upsertPoints(
  ...
  try {
    // 确保 collection 存在
    await createCollection(assistantId);
 ...
 * @param assistantId 助手 ID
 */
export async function getCollectionInfo(assistantId: string): Promise<{
  exists: boolean;
  pointsCount: number;
  vectorsCount: number;
} | null> {
