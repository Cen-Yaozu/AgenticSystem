...
| 领域-对话关联关系 | Image（对应对话内容） |
| 用户权限控制 | Session（消息存储） |
| 文档关联 | Agent（运行时） |

**关键关联**：
- 创建领域时 → 创建 AgentX Container
- 创建对话时 → 创建 AgentX Image，保存 `imageId` 到我们的 conversations 表
...
- 简化 conversations 表结构（添加 imageId，删除 messages 表）
- 创建领域时创建 AgentX Container
- 创建对话时创建 AgentX Image
- 消息发送通过 AgentX Agent
- 对话历史从 AgentX 查询
- WebSocket 代理（转发 AgentX WebSocket）
...
1. **数据迁移**: 现有的 messages 表是否有数据需要迁移？
2. **检索工具 MCP**: 需要实现自定义的检索工具 MCP Server
3. **AgentX 依赖**: 需要在 package.json 中添加 `agentxjs` 依赖
