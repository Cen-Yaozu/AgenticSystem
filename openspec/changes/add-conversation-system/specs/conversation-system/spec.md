...
- AgentX 管理：对话内容（Image、Session、Message）
...
**业务规则**:
- 对话 MUST 关联到一个领域
- 创建对话时 MUST 同时创建 AgentX Image
- 系统 MUST 保存 imageId 到 conversations 表
...
Given 存在领域 "法律顾问"
When 用户请求创建对话
Then 系统创建 AgentX Image
And 系统在 conversations 表保存对话记录
And 返回对话信息包含 id, domainId, imageId, title, status
```
...
- 系统 MUST 同时删除 AgentX Image（级联删除 Session 和 Message）
...
系统 SHALL 为每个领域配置主角色的 systemPrompt。

**业务规则**:
- systemPrompt MUST 包含专业领域定义
- systemPrompt MUST 包含意图识别逻辑
- systemPrompt MUST 包含子代理委派规则
- 创建 AgentX Image 时 MUST 使用配置的 systemPrompt
...
Given 存在领域 "法律顾问"
When 创建对话时
Then 系统使用领域的 systemPrompt 模板
And systemPrompt 包含意图识别和子代理委派规则
And AgentX Image 使用该 systemPrompt
```
...
系统 SHALL 为 AgentX Image 配置 MCP Server，支持用户自定义配置。
...
```gherkin
Given 创建新对话
When 系统创建 AgentX Image
Then Image 配置包含 PromptX MCP Server（系统级）
...
When 系统创建 AgentX Image
Then Image 配置包含用户自定义的 "web-search" MCP
...
**业务规则**:
- 检索决策 MUST 由 AI 根据意图识别自主做出
- 系统 MUST NOT 预先调用检索
- AI MUST 通过检索子代理访问 Qdrant

#### Scenario: AI 自主检索文档

```gherkin
Given 领域 "法律顾问" 有文档 "合同模板.pdf"
And 主角色 systemPrompt 定义了检索子代理
When 用户问 "合同违约条款是什么？"
Then AI 分析意图，决定需要检索
And AI 委派检索子代理
And 检索子代理访问 Qdrant 获取相关文档
And AI 基于文档内容回答
```
...
AI SHALL 自主决定何时检索和保存记忆。

**业务规则**:
- 记忆操作 MUST 由 AI 自主触发
- AI MUST 通过 PromptX MCP 调用 promptx_recall/remember
- 系统 MUST NOT 预先调用记忆操作

#### Scenario: AI 自主检索记忆

```gherkin
Given 领域有用户偏好记忆
And 主角色 systemPrompt 定义了记忆使用规则
When 用户发送消息
Then AI 分析是否需要检索记忆
And AI 自主调用 promptx_recall
And AI 基于记忆内容调整回答
```
...
And 对话包含重要信息
When AI 判断需要保存记忆
Then AI 自主调用 promptx_remember
And 记忆包含对话要点
```

---

### Requirement: CONV-011 子代理委派（Agentic）

AI SHALL 根据意图识别自主委派子代理。

**业务规则**:
- 子代理委派 MUST 由 AI 自主决定
- 委派规则 MUST 在主角色 systemPrompt 中定义
- 系统 MUST NOT 硬编码委派逻辑

#### Scenario: AI 委派检索子代理

```gherkin
Given 主角色 systemPrompt 定义了检索子代理
When 用户问需要文档支持的问题
Then AI 识别意图为"需要检索"
And AI 委派检索子代理执行检索
And AI 整合检索结果生成回答
...
#### Scenario: AI 委派领域专家子代理

```gherkin
Given 主角色 systemPrompt 定义了领域专家子代理
When 用户问专业领域问题
Then AI 识别意图为"需要专业分析"
And AI 委派领域专家子代理
And AI 整合专家分析生成回答
```
...
### Requirement: CONV-014 WebSocket 代理

系统 SHALL 代理 AgentX 的 WebSocket 连接。

**业务规则**:
- 系统 MUST 提供统一的 WebSocket 入口
- 系统 MUST 添加权限验证
- 系统 MUST 转发 AgentX 事件
